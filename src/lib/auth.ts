...
import { DBUser, DBUserInsert } from '@/types/domain';
...
export async function loginUser(email: string, password: string): Promise<string> {
    const user = await queryOne<DBUser>('SELECT id, email, password_hash FROM users WHERE email = $1', [email]);
...
export async function signupUser(email: string, password: string, orgId: string): Promise<string> {
    ...
    const newUser: DBUserInsert = result.rows[0];
